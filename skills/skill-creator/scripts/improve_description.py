...
by calling `hermes -p` as a subprocess (uses the session's auth, no separate
API key needed).
"""
...
def _call_agent(prompt: str, model: str | None, timeout: int = 300) -> str:
    """Run `hermes -p` with the prompt on stdin and return the text response.
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"hermes -p exited {result.returncode}\nstderr: {result.stderr}"
        )
...
- The skill description should focus on the user's intent, what they're trying to achieve, vs. the implementation details.
...
Be creative and mix up the style in different iterations since you'll have multiple opportunities to try different approaches and we'll just grab the highest-scoring one at the end. 
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # Safety net: if the model blew past the 1024-char limit, ask for a shorter rewrite
